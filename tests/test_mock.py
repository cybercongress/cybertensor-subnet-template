...
def test_mock_cwtensor(netuid, n, wallet):
    ...
    if wallet is not None:
        assert cwtensor.is_hotkey_registered(netuid=netuid, hotkey_ss58=wallet.hotkey.ss58_address)
...
def test_mock_dendrite_timings(timeout, min_time, max_time, n):
    ...
    for synapse in responses:
        assert hasattr(synapse, 'dendrite') and type(synapse.dendrite) == ct.TerminalInfo
