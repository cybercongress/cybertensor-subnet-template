...
class StreamMiner(ABC):
    def __init__(self, config=None, axon=None, wallet=None, cwtensor=None):
        ...
        ct.logging.info(self.config)  # TODO: duplicate print?
        ...
        self.wallet = wallet or ct.Wallet(config=self.config)
        ct.logging.info(f"Wallet {self.wallet}")

        # cwtensor manages the blockchain connection, facilitating interaction with the Bittensor blockchain.
        self.cwtensor = cwtensor or ct.cwtensor(config=self.config)
        ct.logging.info(f"Cwtensor: {self.cwtensor}")
        ct.logging.info(
            f"Running miner for subnet: {self.config.netuid} on network: {self.cwtensor.chain_endpoint} with config:"
        )
        ...
        if self.wallet.hotkey.address not in self.metagraph.hotkeys:
            ...
            ct.logging.info(f"Running miner on uid: {self.my_subnet_uid}")

        # The axon handles request processing, allowing validators to send this process requests.
        self.axon = axon or ct.axon(
            wallet=self.wallet, port=self.config.axon.port
        )
        # Attach determiners which functions are called when servicing a request.
        ct.logging.info(f"Attaching forward function to axon.")
    ...
    def run(self):
        ...
        ct.logging.info(
            f"Serving axon {StreamPrompting} on network: {self.config.cwtensor.chain_endpoint} with netuid: {self.config.netuid}"
        )
        ...
        self.last_epoch_block = self.cwtensor.get_current_block()
        ct.logging.info(f"Miner starting at block: {self.last_epoch_block}")

        # This loop maintains the miner's operations until intentionally stopped.
        ct.logging.info(f"Starting main loop")
        step = 0
        try:
            while not self.should_exit:
                ...
                current_block = self.cwtensor.get_current_block()
                while (
                ...
                metagraph = self.cwtensor.metagraph(
                    netuid=self.config.netuid,
                    lite=True,
